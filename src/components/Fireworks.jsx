import { useEffect, useRef } from 'react';
import { fx } from '../fx';

const COLORS = ['#ffd166', '#ff5fa2', '#5ef0ff', '#b388ff', '#ff8a5b', '#9dff8a', '#ffffff'];
const CONFETTI = ['#ffd166', '#ff5fa2', '#5ef0ff', '#b388ff', '#ff8a5b', '#9dff8a'];
const pick = (a) => a[Math.floor(Math.random() * a.length)];

// One full-screen overlay canvas that draws every rocket, spark and confetti piece.
export default function Fireworks() {
  const ref = useRef(null);

  useEffect(() => {
    const canvas = ref.current;
    const ctx = canvas.getContext('2d');
    let w = 0;
    let h = 0;
    let raf = 0;
    const rockets = [];
    const sparks = [];
    const confetti = [];

    function resize() {
      const dpr = Math.min(window.devicePixelRatio || 1, 2);
      w = window.innerWidth;
      h = window.innerHeight;
      canvas.width = w * dpr;
      canvas.height = h * dpr;
      ctx.setTransform(dpr, 0, 0, dpr, 0, 0);
    }

    function burst(x, y, { count = 90, colors, shape } = {}) {
      const palette = colors || [pick(COLORS), pick(COLORS)];
      const kind = shape || (Math.random() < 0.18 ? 'heart' : Math.random() < 0.3 ? 'ring' : 'sphere');
      for (let i = 0; i < count; i++) {
        const a = (i / count) * Math.PI * 2;
        let vx;
        let vy;
        if (kind === 'heart') {
          const hx = 16 * Math.sin(a) ** 3;
          const hy = -(13 * Math.cos(a) - 5 * Math.cos(2 * a) - 2 * Math.cos(3 * a) - Math.cos(4 * a));
          vx = hx * 0.3;
          vy = hy * 0.3;
        } else {
          const speed = kind === 'ring' ? 4.6 + Math.random() * 0.4 : 1.5 + Math.random() * 4.5;
          vx = Math.cos(a + Math.random() * 0.2) * speed;
          vy = Math.sin(a + Math.random() * 0.2) * speed;
        }
        sparks.push({
          x, y, vx, vy,
          life: 1,
          decay: 0.009 + Math.random() * 0.01,
          color: pick(palette),
          size: 1.2 + Math.random() * 1.6,
          twinkle: Math.random() < 0.35,
          drag: kind === 'heart' ? 0.965 : 0.978,
        });
      }
      // bright flash at the center
      sparks.push({ x, y, vx: 0, vy: 0, life: 1, decay: 0.08, color: '#fff', size: 18, flash: true, drag: 1 });
    }

    function rocket(x = w * (0.15 + Math.random() * 0.7), opts = {}) {
      const targetY = h * (0.12 + Math.random() * 0.3);
      const g = 0.09;
      const vy = -Math.sqrt(2 * g * (h - targetY));
      rockets.push({ x, y: h + 10, vx: (Math.random() - 0.5) * 1.2, vy, g, color: pick(COLORS), opts });
    }

    function addConfetti(count) {
      for (let i = 0; i < count; i++) {
        confetti.push({
          x: Math.random() * w,
          y: -20 - Math.random() * h * 0.5,
          vx: (Math.random() - 0.5) * 2,
          vy: 2 + Math.random() * 3,
          rot: Math.random() * Math.PI,
          vr: (Math.random() - 0.5) * 0.25,
          wobble: Math.random() * Math.PI * 2,
          size: 6 + Math.random() * 7,
          color: pick(CONFETTI),
          circle: Math.random() < 0.25,
        });
      }
    }

    function sparkle(x, y) {
      if (sparks.length > 2500) return;
      sparks.push({
        x, y,
        vx: (Math.random() - 0.5) * 1.2,
        vy: (Math.random() - 0.5) * 1.2 + 0.4,
        life: 0.9,
        decay: 0.03,
        color: pick(['#ffd166', '#ff9fd0', '#9ff6ff', '#ffffff']),
        size: 1.5 + Math.random() * 1.5,
        twinkle: true,
        drag: 0.96,
      });
    }

    function frame() {
      ctx.clearRect(0, 0, w, h);
      ctx.globalCompositeOperation = 'lighter';

      for (let i = rockets.length - 1; i >= 0; i--) {
        const r = rockets[i];
        r.x += r.vx;
        r.y += r.vy;
        r.vy += r.g;
        ctx.globalAlpha = 1;
        ctx.strokeStyle = r.color;
        ctx.lineWidth = 2.5;
        ctx.beginPath();
        ctx.moveTo(r.x, r.y);
        ctx.lineTo(r.x - r.vx * 3, r.y - r.vy * 3);
        ctx.stroke();
        if (Math.random() < 0.6) sparkle(r.x, r.y + 4);
        if (r.vy >= -0.5) {
          rockets.splice(i, 1);
          burst(r.x, r.y, r.opts);
        }
      }

      for (let i = sparks.length - 1; i >= 0; i--) {
        const s = sparks[i];
        s.vx *= s.drag;
        s.vy = s.vy * s.drag + (s.flash ? 0 : 0.045);
        s.x += s.vx;
        s.y += s.vy;
        s.life -= s.decay;
        if (s.life <= 0) {
          sparks.splice(i, 1);
          continue;
        }
        const alpha = s.twinkle ? s.life * (0.4 + Math.random() * 0.6) : s.life;
        ctx.globalAlpha = alpha;
        if (s.flash) {
          const g = ctx.createRadialGradient(s.x, s.y, 0, s.x, s.y, s.size * 3);
          g.addColorStop(0, 'rgba(255,255,255,0.9)');
          g.addColorStop(1, 'rgba(255,255,255,0)');
          ctx.fillStyle = g;
          ctx.fillRect(s.x - s.size * 3, s.y - s.size * 3, s.size * 6, s.size * 6);
        } else {
          ctx.strokeStyle = s.color;
          ctx.lineWidth = s.size;
          ctx.lineCap = 'round';
          ctx.beginPath();
          ctx.moveTo(s.x, s.y);
          ctx.lineTo(s.x - s.vx * 2.5, s.y - s.vy * 2.5);
          ctx.stroke();
        }
      }

      ctx.globalCompositeOperation = 'source-over';
      for (let i = confetti.length - 1; i >= 0; i--) {
        const c = confetti[i];
        c.wobble += 0.08;
        c.x += c.vx + Math.sin(c.wobble) * 1.2;
        c.y += c.vy;
        c.rot += c.vr;
        if (c.y > h + 30) {
          confetti.splice(i, 1);
          continue;
        }
        ctx.globalAlpha = 1;
        ctx.fillStyle = c.color;
        ctx.save();
        ctx.translate(c.x, c.y);
        ctx.rotate(c.rot);
        ctx.scale(1, Math.cos(c.wobble * 1.5));
        if (c.circle) {
          ctx.beginPath();
          ctx.arc(0, 0, c.size / 2.4, 0, Math.PI * 2);
          ctx.fill();
        } else {
          ctx.fillRect(-c.size / 2, -c.size / 4, c.size, c.size / 2);
        }
        ctx.restore();
      }

      ctx.globalAlpha = 1;
      raf = requestAnimationFrame(frame);
    }

    resize();
    window.addEventListener('resize', resize);
    fx.register({ burst, rocket, confetti: addConfetti, sparkle });
    raf = requestAnimationFrame(frame);
    return () => {
      cancelAnimationFrame(raf);
      window.removeEventListener('resize', resize);
      fx.register(null);
    };
  }, []);

  return <canvas ref={ref} className="fireworks" aria-hidden="true" />;
}
